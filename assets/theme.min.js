                ...
                (m.prototype = Object.assign({}, m.prototype, {
                    _onSelectChange: function () {
                      ...
                      if (variant.compare_at_price && variant.compare_at_price > variant.price) {
                        ...
                        const badge = document.querySelector('.t4s-badge-sale+.t4s-badge-price');
                        if (badge) {
                    ...
                    //             this.$productPrice.html(e +  '<span class="t4s-badge-price">Final Sale</span>');
                    ...
                    _updatePrice: function (t, e, i) {
                        ...
                        const formatCurrency = (amount) => {
                            let formatted = T4SThemeSP.Currency.formatMoney(amount);
                            // Detect active currency (from Shopify theme or your app)
                            let currentCurrency = T4SThemeSP.Currency.currentCurrency || Shopify.currency.active || 'USD';
                            // Handle AUD → AU$ (prefix only)
                            if (currentCurrency === 'AUD') {
                                formatted = formatted.replace(/\s?AUD/i, '').trim(); // remove any trailing AUD
                                formatted = formatted.replace(/^\$/, 'AU$');         // replace $ with AU$
                            }

                            // Handle GBP → remove GBP suffix
                            if (currentCurrency === 'GBP') {
                                formatted = formatted.replace(/\s?GBP/i, '').trim(); // remove trailing GBP
                            }
                            return formatted;
                        };

                        let s = formatCurrency(n);

                        if ((this.$productPrice.show(), this.variantState.onSale)) {
                            let tFormatted = formatCurrency(a),
                                e = this.formartPrice.replace("money_ins", s).replace("money_del", tFormatted),
                                i = a - n,
                                o = (100 * i) / a,
                                r = Math.round(o);
                            ...
                            if ("1" == this.saletype) {
                                e += ` <span class="t4s-badge-price">${f.replace("[sale]", r)}</span>`;
                            } else if ("2" == this.saletype) {
                                e += ` <span class="t4s-badge-price">${p.replace("[sale]", formatCurrency(i))}</span>`;
                            }
                        ...
                        if (this.variantState.showUnitPrice) {
                            this.$unit_price.html(formatCurrency(t.unit_price));
                    ...
                    _updateDelivery: function (t, e, i) {
                        ...
                        let id = t.id;
                        let note = i.find('.t4s-product__select option').filter(function() {
                            return this.value == id;
                        }).data('note');
                      if(note) {
                        n.find('.t4s-pr_delivery_content').text(note);
                      } else {
                        n.find('.t4s-pr_delivery_content').text( n.find('.t4s-pr_delivery_content').data('product-note'));
                      }
                        if (n[0])
                            if (t && t.available) {
                                var a = S(n.attr("data-order-delivery"));
                                this.variantState.preOrder && a.hideWithPreorder ? n.show() : n.hide();
                            } else n.show();
                    },
